cloudera.hosts.self.commission() {
  # description

  # variables
  local _api_version=v19
  local _commission_requested=${false}
  local _count=0
  local _json=
  ...
  while ( 
    ...
    # set status
    if  [[ ${_state[decommissioned]} == ${true} ]] &&   \
        [[ ${_commission_requested} == ${false} ]]; then
      _json=$( json.set --json "${_json}" --key .commission.data --value "$( cloudera.api --api /cm/commands/hostsRecommissionWithStart ${@} )" )
      _commission_requested=${true}
      [[ ${VERBOSE} == ${true} ]] && >&2 ${cmd_echo} $( date.pretty ) - commission: requested
    
    elif [[ ${_state[commissioned]} == ${true} ]]; then
      _json=$( json.set --json "${_json}" --key .state.is_commissioned --value ${true} )
      _json=$( json.set --json "${_json}" --key .state.date --value $( date.epoch ) )

      [[ ${VERBOSE} == ${true} ]] && >&2 ${cmd_echo} $( date.pretty ) - commission: successful
    ...
    (( _count++ ))
    [[ ${_state[commissioned]} == ${false} ]] && ${cmd_sleep} $(( ${_sleep} * 60 ))
  done
