move.validate.key.coriolis_transfer_destination_os-release() {
  # local variables
  local _count=0
  local _endpoint_destination=
  local _key=.coriolis.transfer.destination.os_release
  local _type=os_release
  ...
  if [[ ! -z ${_endpoint_destination} ]]; then
    ...
    if [[ $( ${cmd_echo} ${_json} | ${cmd_jq} "${_key}" ) != null ]]; then
      ...
      if [[ $( move.coriolis.list.endpoints.destination.options --endpoint ${_endpoint_destination} --type ${_type} | ${cmd_jq} '[ .[] | select( .id == "'"$( ${cmd_echo} ${_json} | ${cmd_jq} -r "${_key}" )"'") ] | length' ) > 0 ]]; then
        # value exists
        if [[ ${_naemon} == ${false} ]]; then
        ...
        if [[ $( ${cmd_echo} ${_json} | ${cmd_jq} "${_key}" ) != null ]]; then
          if [[ ${_naemon} == ${false} ]]; then
            ...
            shell.log "${_exit_string}${FUNCNAME}(${_profile}) - [$( ${cmd_echo} ${_type} | ucase )]  $( ${cmd_echo} $( move.coriolis.list.endpoints.destination.options --endpont ${_endpoint_destination} --type ${_type} --output id ) | ${cmd_sed} 's/\ /, /g' )"
            ...
            _exit_string+="\n[$( ${cmd_echo} ${_type} | ucase )]  $( ${cmd_echo} $( move.coriolis.list.endpoints.destination.options --endpont ${_endpoint_destination} --type ${_type} --output id ) | ${cmd_sed} 's/\ /, /g' )"
            _exit_string+="\n\n"
      ...
      if [[ ${_naemon} == ${false} ]]; then
        ...
        shell.log "${FUNCNAME}(${_profile}) - [$( ${cmd_echo} ${_type} | ucase )]  $( ${cmd_echo} $( move.coriolis.list.endpoints.destination.options --endpont ${_endpoint_destination} --type ${_type} --output id ) | ${cmd_sed} 's/\ /, /g' )" 
