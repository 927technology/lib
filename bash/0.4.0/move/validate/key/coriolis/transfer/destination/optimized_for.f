move.validate.key.coriolis_transfer_destination_optimized_for() {
  # local variables
  local _count=0
  ...
  if [[ ! -z ${_endpoint_destination} ]]; then
    ...
    if [[ $( ${cmd_echo} ${_json} | ${cmd_jq} "${_key}" ) != null ]]; then
      ...
      if [[ $( move.coriolis.list.endpoints.destination.options --endpoint ${_endpoint_destination} --type ${_type} --profile ${_profile} | ${cmd_jq} '. | length' ) > 0 ]]; then
        # value exists
        ...
        if [[ $( ${cmd_echo} ${_json} | ${cmd_jq} "${_key}" ) != null ]]; then
          ...
          shell.log "${_exit_string}${FUNCNAME}(${_profile}) - [$( ${cmd_echo} ${_type} | ucase )]  $( ${cmd_echo} $( move.coriolis.list.endpoints.destination.options --endpont ${_endpoint_destination} --type ${_type} --profile ${_profile} | ${cmd_jq} -r '.[]' ) | ${cmd_sed} 's/\ /, /g' )"
      ...
      shell.log "${FUNCNAME}(${_profile}) - [$( ${cmd_echo} ${_type} | ucase )]  $( ${cmd_echo} $( move.coriolis.list.endpoints.destination.options --endpont ${_endpoint_destination} --type ${_type} --profile ${_profile} | ${cmd_jq} -r '.[]' ) | ${cmd_sed} 's/\ /, /g' )" 
