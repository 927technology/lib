move.validate.key.coriolis_transfer_destination_optimized-for() {
  # local variables
  local _count=0
  local _endpoint_destination=
  local _key=.coriolis.transfer.destination.optimized_for
  local _type=optimized_for
  ...
  if [[ ! -z ${_endpoint_destination} ]]; then
    ...
    if [[ $( ${cmd_echo} ${_json} | ${cmd_jq} "${_key}" ) != null ]]; then
      ...
      if [[ $( move.coriolis.list.endpoints.destination.options --endpoint ${_endpoint_destination} --type ${_type} | ${cmd_jq} '. | length' ) > 0 ]]; then
        # value exists
        if [[ ${_naemon} == ${false} ]]; then
        ...
        if [[ $( ${cmd_echo} ${_json} | ${cmd_jq} "${_key}" ) != null ]]; then
          if [[ ${_naemon} == ${false} ]]; then
            ...
            shell.log "${_exit_string}${FUNCNAME}(${_profile}) - [$( ${cmd_echo} ${_type} | ucase )]  $( ${cmd_echo} $( move.coriolis.list.endpoints.destination.options --endpont ${_endpoint_destination} --type ${_type} | ${cmd_jq} -r '.[]' ) | ${cmd_sed} 's/\ /, /g' )"
            ...
            _exit_string+="\n[$( ${cmd_echo} ${_type} | ucase )]  $( ${cmd_echo} $( move.coriolis.list.endpoints.destination.options --endpont ${_endpoint_destination} --type ${_type} | ${cmd_jq} -r '.[]' ) | ${cmd_sed} 's/\ /, /g' )"
            _exit_string+="\n\n"
      ...
      if [[ ${_naemon} == ${false} ]]; then
        ...
        shell.log "${FUNCNAME}(${_profile}) - [$( ${cmd_echo} ${_type} | ucase )]  $( ${cmd_echo} $( move.coriolis.list.endpoints.destination.options --endpont ${_endpoint_destination} --type ${_type} | ${cmd_jq} -r '.[]' ) | ${cmd_sed} 's/\ /, /g' )" 
