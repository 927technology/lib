move.validate.key.coriolis_transfer_destination_leave-migrated-vms-off() {
  # local variables
  local _count=0
  local _endpoint_destination=
  local _key=.coriolis.transfer.destination.leave_migrated_vms_off
  local _type=boolean
  ...
  if [[ ! -z ${_endpoint_destination} ]]; then
    ...
    if [[ $( ${cmd_echo} ${_json} | ${cmd_jq} "${_key}" ) != null ]]; then
      ...
      if [[ $( ${cmd_echo} [\"true\",\"false\"] | ${cmd_jq} '[ .[] | select( . == "'"$( ${cmd_echo} ${_json} | ${cmd_jq} -r "${_key}" )"'") ] | length' ) > 0 ]]; then
        if [[ ${_naemon} == ${false} ]]; then
        ...
        if [[ $( ${cmd_echo} ${_json} | ${cmd_jq} "${_key}" ) != null ]]; then
          if [[ ${_naemon} == ${false} ]]; then
            ...
            shell.log "${_exit_string}${FUNCNAME}(${_profile}) - [$( ${cmd_echo} ${_type} | ucase )]  true, false"
            ...
            _exit_string+="\n[$( ${cmd_echo} ${_type} | ucase )] true, false"
            _exit_string+="\n\n"
      ...
      if [[ ${_naemon} == ${false} ]]; then
        ...
        shell.log "${FUNCNAME}(${_profile}) - [$( ${cmd_echo} ${_type} | ucase )]  true, false" 
