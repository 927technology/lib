move.naemon.create.hosts () {
  ...
  ## main
  # create hosts folder
  if [[ ! -d ${_path_naemon}/conf.d/hosts/${_profile} ]]; then
    shell.log "${FUNCNAME}(${_profile}) - [CREATING] ${_path_naemon}/conf.d/hosts/${_profile}"
    ${cmd_mkdir} --parents ${_path_naemon}/conf.d/hosts/${_profile}
  fi

  # move files ensuring deleted hosts are not carried forward
  if [[ ! -d ${_path_naemon}/conf.d/hosts/${_profile}/.delete ]]; then
    shell.log "${FUNCNAME}(${_profile}) - [CREATING] ${_path_naemon}/conf.d/hosts/${_profile}/.delete"
    ${cmd_mkdir} --parents ${_path_naemon}/conf.d/hosts/${_profile}/.delete
  fi

  shell.log "${FUNCNAME}(${_profile}) - [MOVING] ${_path_naemon}/conf.d/hosts/${_profile}/*.cfg to ${_path_naemon}/conf.d/hosts/${_profile}/.delete"
  ${cmd_mv} ${_path_naemon}/conf.d/hosts/${_profile}/*.cfg ${_path_naemon}/conf.d/hosts/${_profile}/.delete/ > /dev/null 2>&1




  # for host in $( ${cmd_echo} "${_json}" | ${cmd_jq} -c '.[]' ); do 
  for host in $( move.list.transfers --filter "(.*)" --profile ${_profile} | ${cmd_jq} -c '.[] | if( .move.coriolis.enable == 1 ) then . else empty end' ); do 
    ...
    _address=$(                         ${cmd_echo} "${host}"  | ${cmd_jq} -r  'try( .GuestIP[0] )                              | if( . == null ) then "" else . end' )
    
    _alias=$(                           ${cmd_echo} "${host}"  | ${cmd_jq} -r  'try( .vsphere.host.id )                              | if( . == null ) then "" else . end' )
    ...
    _coriolis_host_json=$(              ${cmd_cat} ~move/move/${_profile}/transfers/vms/"${_alias}".json 2>/dev/null | ${cmd_jq} -c )
    _coriolis_transfer_json=$(          ${cmd_cat} ~move/move/${_profile}/transfers/"${_alias}".json 2>/dev/null | ${cmd_jq} -c )
    _vsphere_host_json=$(               ${cmd_cat} ~move/vsphere/${_profile}/vms/"${_alias}".json 2>/dev/null | ${cmd_jq} -c )
    ...
    _file_name=$(                       ${cmd_echo} "${host}"  | ${cmd_jq} -r  'try( .name )                                    | if( . == null ) then "" else . end' ) 
    ...
    _host_name=$(                       ${cmd_echo} "${host}"  | ${cmd_jq} -r  'try( .name )                              | if( . == null ) then "" else . end' )
    
    # _hostgroups=$(                      ${cmd_echo} "${host}"  | ${cmd_jq}     'try( .ops[0].hostgroups[] )               | select( .enable == true ).name' | ${cmd_jq} -sr '. | if( . | length < 1 ) then "" else join(", ") end' )
    _hostgroups=$(                      ${cmd_cat} /usr/local/etc/coriolis/osfamily.json  | ${cmd_jq} -r '[ .osfamily[ [ .osfamily[].vsphere | any(. =="'"$( ${cmd_echo} ${host} | ${cmd_jq} -r '.vsphere.host.guestos' )"'") ] | index(true) ] ][0].family' )
    ...
    shell.log "${FUNCNAME}(${_profile}) - [WRITING] ${_path_naemon}/conf.d/hosts/${_profile}/${_file_name}.cfg"


    # get hash if file is present
    if [[ -f ${_path_naemon}/conf.d/hosts/${_profile}/.delete/${_file_name}.cfg ]]; then
      shell.log "${FUNCNAME}(${_profile}) - [HASHING] ${_path_naemon}/conf.d/hosts/${_profile}/.delete/${_file_name}.cfg"
      _file_hash=$( ${cmd_sha256sum} ${_path_naemon}/conf.d/hosts/${_profile}/.delete/${_file_name}.cfg | ${cmd_awk} '{print $1}' )    
    
    else
      shell.log "${FUNCNAME}(${_profile}) - [HASHING] ${_path_naemon}/conf.d/hosts/${_profile}/.delete/${_file_name}.cfg does not exist"
    ...
    # write file
    ${cmd_cat} << EOF.host > ${_path_naemon}/conf.d/hosts/${_profile}/${_file_name}.cfg
define host                        {
...
$( ${cmd_printf} '%-1s %-32s %-50s' "" hostgroups "vsphere.hosts, ${_hostgroups}.hosts" )
...
$( [[ -z ${_use} ]]                           && ${cmd_printf} '%-1s %-32s %-50s' "" use "generic-vsphere" )


$( ${cmd_printf} '%-1s %-32s %-50s' "" _coriolis_host_json  \'"${_coriolis_host_json}"\' )
$( ${cmd_printf} '%-1s %-32s %-50s' "" _coriolis_transfer_json  \'"${_coriolis_transfer_json}"\' )
$( ${cmd_printf} '%-1s %-32s %-50s' "" _vsphere_host_json  \'"${_vsphere_host_json}"\' )

$( ${cmd_printf} '%-1s %-32s %-50s' "" _profile  "${_profile}" )
...
EOF.host
    ...
    # check hash
    if  [[ ${_file_hash} == $( ${cmd_sha256sum} ${_path_naemon}/conf.d/hosts/${_profile}/${_file_name}.cfg | ${cmd_awk} '{print $1}' ) ]]; then
        (( _restart_naemon_count++ ))
    fi

    ${cmd_sed} -i '/^[[:space:]]*$/d' ${_path_naemon}/conf.d/hosts/${_profile}/${_file_name}.cfg 2>/dev/null
  done 

  # restart naemon
  if [[ ${_restart_naemon_count} > 0 ]]; then
    shell.log "${FUNCNAME}(${_profile}) - [RESTART] Naemon: changes detected"
    ...
    shell.log "${FUNCNAME}(${_profile}) - [RESTART] Naemon: no changes detected"

  fi

  # cleanup ${_path_naemon}/conf.d/hosts/${_profile}/.delete
  shell.log "${FUNCNAME}(${_profile}) - [CLEANUP] ${_path_naemon}/conf.d/hosts/${_profile}/.delete"

  ${cmd_rm} --recursive --force ${_path_naemon}/conf.d/hosts/${_profile}/.delete
