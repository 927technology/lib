move.coriolis.create.deployment() {
  # edited
  # chris murray
  # 20251120

  # description
  # creates coriolis deployment set in the ${_path}/${_profile}/transfers/${_name}.json 
  # writes output to ${_path}/${_profile}/deployments/${_id}.json as json -c format
  ...
  # set deployment in coriolis server
  _json=$( ${cmd_coriolis} deployment create --format json ${_id_transfer} || (( _error_count++ )) )

  # write deployment output to file
  ${cmd_echo} "${_json}" > ${_path}/${_profile}/deployments/$( ${cmd_echo} ${network} | ${cmd_jq} -r '.ID' ).json
  ${cmd_ln} -fs $( ${cmd_echo} ${network} | ${cmd_jq} -r '.ID' ).json ${_path}/${_profile}/deployments/${_name}
