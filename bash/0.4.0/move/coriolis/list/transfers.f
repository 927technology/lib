move.coriolis.list.transfers() {
  ...
  # main
  if  [[ ! -z  ${_name} ]]            && \
      [[ -d ${_path}/${MOVE_PROFILE}/transfers ]]; then
    _json=$( ${cmd_cat} ${_path}/${MOVE_PROFILE}/transfers/*.json 2>/dev/null | ${cmd_jq} '. | select(.Notes | startswith( "'"${_name} "'" ))' && _exit_code=${exit_ok} || _exit_code=${exit_crit} )

  elif  [[ -d ${_path}/${MOVE_PROFILE}/transfers ]]; then
    _json=$( ${cmd_cat} ${_path}/${MOVE_PROFILE}/transfers/*.json 2>/dev/null | ${cmd_jq} -c && _exit_code=${exit_ok} || _exit_code=${exit_crit} )
