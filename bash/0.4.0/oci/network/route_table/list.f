oci.network.route_table.list() {
  ...
  for compartment in $( ${cmd_echo} ${_json_compartments} | ${cmd_jq} -r '.[].id'); do
    ...
    # get route_tables in each compartment
    for route_table in $( ${cmd_oci} network route-table list --compartment-id ${compartment} --profile ${_profile} | ${cmd_jq} -c '.data[]' ); do
      _route_table_id=$( ${cmd_echo} ${route_table} | ${cmd_jq} -r '.id' )
      
      # add route_table to json
      _json_route_table[${_count_route_table}]="${route_table}"
      [[ ${?} == ${exit_ok} ]] && _exit_code=${exit_ok} || _exit_code=${exit_crit}
      
      # add compartment to route_table in json
      _json_route_table[${_count_route_table}]=$( json.set --json ${_json_route_table[${_count_route_table}]} --key .compartment --value "$( ${cmd_echo} ${_json_compartments} | ${cmd_jq} -c '.['${_count_compartment}']' )" )
      [[ ${?} == ${exit_ok} ]] && _exit_code=${exit_ok} || _exit_code=${exit_crit}
  ...
  # build json list form _json_route_table array
