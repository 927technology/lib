927.mod_gearman.create.module() {
  ...
  if [[ ! -z "${_json}" ]] && [[ $( ${cmd_echo} "${_json}" | ${cmd_jq} '.[] | length' ) > 0 ]]; then
    ...
    for jobserver in $(                   ${cmd_echo} "${_json}"        | ${cmd_jq} -c '.[] | select(( .enable == true ) and .ops[0].id == "'${SERVER_ID}'")' ); do 
      ...
      # write file
      if shell.create.file --group root --owner naemon --path ${_path_gearmand}/module.conf; then

        shell.log --screen --message "WRITING: ${_path_gearmand}/module.conf"
