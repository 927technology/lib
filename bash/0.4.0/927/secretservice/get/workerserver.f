927.secretservice.get.workerserver () {
  # description
  ...
  if [[ ! -z ${_group} ]] && [[ ! -z ${_owner} ]] && [[ -d ${_path} ]] && [[ ! -z ${_secrets_provider} ]]; then
    case ${_secrets_provider} in
      bitwarden )
        ...
        _workerserver_pass=$( ${cmd_bws} secret list | ${cmd_jq} -r '.[] | select(.key == "workerserver_pass").value' )
      ;;
    esac

    echo $_workerserver_pass

    if [[ ! -f ${_path}/secrets/workerserver.pwd ]] || \
       [[ $( ${cmd_cat} ${_path}/secrets/workerserver.pwd | ${cmd_sha256sum} ) != $( ${cmd_echo} ${_workerserver_pass} | ${cmd_sha256sum} ) ]]; then
      # ensure user name and password are not blank
      if [[ ! -z ${_workerserver_pass} ]]; then
        ...
        # write password file
        ${cmd_echo} ${_workerserver_pass} > ${_path}/secrets/workerserver.pwd

        # set owner, group, and mode
        ${cmd_chown} ${_owner}:${_group} ${_path}/secrets/workerserver.pwd
        ${cmd_chmod} 600 ${_path}/secrets/workerserver.pwd
