927.naemon.create.serviceescalations () {
  # description
  ...
  if [[ ! -z ${_json} ]] && [[ $( ${cmd_echo} ${_json} | ${cmd_jq} '. | length' ) > 0 ]]; then
    ...
    for serviceescalation in $( ${cmd_echo} "${_json}" | ${cmd_jq} -c '.[] | select(.enable == true)' ); do 
      ...
      _contacts=$(              ${cmd_echo} ${servicedependancy}  | ${cmd_jq}    'try( .ops[0].contacts[] )                     | select( .enable == true ).string' | ${cmd_jq} -sr '. | if( . | length < 1 ) then "" else join(", ") end' )
    
      _contact_groups=$(        ${cmd_echo} ${servicedependancy}  | ${cmd_jq}    'try( .ops[0].contactgroups[] )                | select( .enable == true ).string' | ${cmd_jq} -sr '. | if( . | length < 1 ) then "" else join(", ") end' )
    
      _escalation_options=$(    ${cmd_echo} ${servicedependancy}  | ${cmd_jq}    'try( .ops[0].escalation.options )             | to_entries[] | select(.value == true) | .key[0:1]' | ${cmd_jq} -sr '. | if( . | length < 1 ) then "" else join(", ") end' )
    
      _escalation_period=$(     ${cmd_echo} ${servicedependancy}  | ${cmd_jq} -r 'try( .ops[0].escalation.timeperiod.string )   | select( .escalation.timeperiod.enable == true )' )
    
      _file_name=$(             ${cmd_echo} ${servicedependancy}  | ${cmd_jq} -r 'try( .ops[0].name.string )' )
    
      _first_notification=$(    ${cmd_echo} ${servicedependancy}  | ${cmd_jq} -r 'try( .ops[0].first_notification )' )
    
      _host_name=$(             ${cmd_echo} ${servicedependancy}  | ${cmd_jq} -r 'try( .ops[0].name.string )' )
      ...
      _last_notification=$(     ${cmd_echo} ${servicedependancy}  | ${cmd_jq} -r 'try( .ops[0].last_notificaton )' )
    
      _notification_interval=$( ${cmd_echo} ${servicedependancy}  | ${cmd_jq} -r 'try( .ops[0].notification_interval )' )
