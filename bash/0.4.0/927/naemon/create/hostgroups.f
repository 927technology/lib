927.naemon.create.hostgroups () {
  ...
  if [[ ! -z ${_json} ]] && [[ $( ${cmd_echo} ${_json} | ${cmd_jq} '. | length' ) > 0 ]]; then
    ...
    for hostgroup in $( ${cmd_echo} ${_json} | ${cmd_jq} -c '.[] | select(.enable == true)' ); do 
      ...
      _hostgroup_name=$(                ${cmd_echo} ${hostgroup}  | ${cmd_jq} -r  'try( .ops[0].name.string )                   | if( . == null ) then "" else . end' )

      _hostgroup_members=$(             ${cmd_echo} ${hostgroup}  | ${cmd_jq}     'try( .ops[0].hostgroups[] )                  | select( .enable == true ).name' | ${cmd_jq} -sr '. | if( . | length < 1 ) then "" else join(", ") end' )
