927.ops.create.workerserver () {
  ...
  if [[ ${_verbose} == ${true} ]]; then
    echo path $_path
    # echo length ${_json} | jq '. | length'
    echo worker id $WORKER_ID
    # echo enable $_json | jq '.[0].enable'
    # echo config id $_json | jq '.[0].ops[0].id'
  fi

  if [[ ! -z "${_json}" ]] && [[ $( ${cmd_echo} "${_json}" | ${cmd_jq} '.[] | length' ) > 0 ]]; then
    # [[ ! -d ${_path} ]] && ${cmd_mkdir} -p ${_path} || ${cmd_rm} -rf ${_path}/*

    for workerserver in $(                ${cmd_echo} "${_json}"    | ${cmd_jq} -c '.[] | select(( .enable == true ) and .ops[0].id == "'${WORKER_ID}'")' ); do
      [[ ${_verbose} == ${true} ]] && ${cmd_echo} "${workerserver}"
      ...
      _hostgroups=$(                      ${cmd_echo} "${workerserver}"   | ${cmd_jq} -r  'try( .ops[0].host.groups[] )                 | if( . | length > 1 ) then "" else join(", ") end' )

      _hosts=$(                           ${cmd_echo} "${workerserver}"   | ${cmd_jq} -r  'try( .ops[0].hosts.enable )                  | if( . == '${true}' ) then "yes" else "no" end' )
      ...
      _keyfile=$(                         ${cmd_echo} "${workerserver}"   | ${cmd_jq} -r  'try( .ops[0].encryption.keyfile )            | if( . == null ) then "secrets/module.pwd" else . end' )
      ...
      # _pid_file=$(                        ${cmd_echo} "${workerserver}"   | ${cmd_jq} -r  'try( .ops[0].p1_file )                       | if( . == null ) then "/var/mod_gearman/mod_gearman_worker.pid" else . end' )
...
use_embedded_perl_implicitly=${_use_embedded_pearl_implicitly}

use_perl_cache=${_use_pearl_cache}

workaround_rc_25=${_workaround_rc_25}
EOF.config

      [[ ${?} != ${exit_ok} ]] && (( _error_count++ ))
      ${cmd_sed} -i '/^[[:space:]]*$/d' ${_path}/worker.conf
    done
